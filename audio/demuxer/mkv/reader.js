...
class MatroskaReader{
	...
	parse(element, cb){
		this.reader.read(element.size, (err, reader) => {
			if(err)
			...
			if(reader){
				if(element.id == util.MatroskaID.EBMLHeader){
					...
					if(header.docType != "matroska" && header.docType != "webm")
						return this.finish(new Error(Errors.EBML_WRONG_FORMAT));
	...
	readElement(cb){
		...
		util.async.readVint(this.reader, false, (err, id, id_read) => {
			if(err)
				return cb(err, null);
			if(id === null)
				cb(null, null);
			else
				util.async.readVint(this.reader, false, (err, size, size_read) => {
					if(err)
						return cb(err, null);
					if(size === null)
						cb(null, null);
	...
	finish(error){
		...
		if(this.tracks && this.first_cluster_offset)
			...
			this.cb(Errors.MISSING_TRACK_DATA);
	}
}

Codec.register('A_OPUS', 'opus');
Codec.register('A_AAC', 'aac');
Codec.register('A_VORBIS', 'vorbis');
...
module.exports = {
	probe(stream, cb){
		...
		return util.sync.readVint(reader, false) == util.MatroskaID.EBMLHeader;
	}, read(stream, cb){
