	...
	constructor(file, track, sidx, moofs){
		...
		if(moofs){
			...
			if(moof){
				...
				process.nextTick(() => {
					this.emit('error', ERR_TRACK_NOT_IN_MOOF);
				});
	...
	processSegment(seg, cb, min_duration = 0){
		...
		var proc = () => {
			...
			sreader.read(8, (reader) => {
				...
				if(size == 0 || type != 'moof')
					return cb(ERR_UNEXPECTED_BOX, null);
				sreader.read(size - 8, (reader) => {
					...
					if(!moof)
						return cb(ERR_TRACK_NOT_IN_MOOF, null);
					var err = this.handleMoof(moof);
	...
	processMoof(sreader, moof, cb){
		...
		sreader.read(size, (reader) => {
			if(aborted)
				return;
			if(reader)
				cb(null, {data: reader.read(size), time_offset: moof.base_media_decode_time, samples: moof.samples}, true);
			else
				cb(ERR_UNEXPECTED_READ_END, null);
			if(destroy_reader)
				sreader.destroy();
	...
	seek(time){
		if(this.sidx){
			...
			while(l < r){
				...
				if(moof && moof.base_media_decode_time > time)
					r = mid - 1;
				else
					l = mid;
			}

			if(l == this.moofs.length && this.moof_fetching)
