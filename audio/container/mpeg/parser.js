...
class MpegParser{
	...
	parseSegmentIndex(reader){
		...
		var bytestart = new Uint32Array(length);
		var byteend = new Uint32Array(length);
		var timestart = new Uint32Array(length);
		var timelength = new Uint32Array(length);

		for(var i = 0; i < length; i++){
			var pb = curbyte;
			var pt = curtime;
			var lb = reader.readUint32() & 0x7fffffff;
			var lt = reader.readUint32();
		...
		return {timescale, time: {start: timestart, length: timelength}, byte: {start: bytestart, end: byteend}, length};
	}
